...
export function describeNotification(notification: Notification): string {
  ...
  switch (notification.type) {
    ...
    case "task_status_changed":
      return `"${data.taskTitle ?? "A task"}" changed to ${data.status ?? "a new status"}`;
    case "mentioned_in_comment":
      return `You were mentioned in a comment on "${data.taskTitle ?? "a task"}"`;
    case "comment_added":
      return `New comment on "${data.taskTitle ?? "a task"}"`;
    case "added_to_project":
      return `You were added to "${data.projectName ?? "a project"}"`;
    case "organization_invitation":
      return `You were invited to join "${data.organizationName ?? "an organization"}"`;
    case "organization_role_changed":
      return `Your role in "${data.organizationName ?? "an organization"}" changed to ${
        typeof data.role === "string"
          ? formatEnumLabel(data.role)
          : "a new role"
      }`;
    case "removed_from_organization":
      return `You were removed from "${data.organizationName ?? "an organization"}"`;
